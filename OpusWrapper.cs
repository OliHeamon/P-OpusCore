...
{
    private const string DllPath = "libopus";
    ...
    [DllImport(DllPath, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
    private static extern int opus_encode(IntPtr st, ref short pcm, int frame_size, byte[] data, int max_data_bytes);

    public static int opus_encode(IntPtr st, Span<short> pcm, int frame_size, byte[] data)
    {
        if (st == IntPtr.Zero)
            throw new ObjectDisposedException("OpusEncoder");

        int payloadLength = opus_encode(st, ref pcm[0], frame_size, data, data.Length);
